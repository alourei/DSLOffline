...
//2016/05/30 22:00 Added total energy spectra on si form alphas and He3
//2016/05/29 15:00 Added addback spectra
//2016/05/29 19:00 Added addback PID Gamma gated matrix
//2016/05/29 18:09 Progress also shown as a percentage
//2016/05/29 19:09 Added the total Run time;
...
Int_t main(Int_t argc,Char_t **argv){
  ...
  if(argc<4){
    ...
    Char_t outname[256];
    sprintf(outname,"DSL_output_%s",argv[3]);
   ...
   TH2F *PID_gated_5540=new TH2F("PID_g","#DeltaE-E PID plot gated on 5545 keV;E;#Delta E",200,0,5000,200,0,5000);
   TH2F *PID_gated_5292=new TH2F("PID_gh","#DeltaE-E PID plot gated on 5292 keV;E;#Delta E",200,0,5000,200,0,5000);
   ...
   TH1F *hGriffin0g_alpha_ex=new TH1F("hGriff_0g_alpha_ex","Total Energy in Griffin 0 deg Gated on high alpha energy (low excitation)",8000/4,0,8000);
   TH1F *hGriffin90g_alpha_ex=new TH1F("hGriff_90g_alpha_ex","Total Energy in Griffin 90 deg Gated on high alpha energy (low excitation)",8000/4,0,8000);
   ...
   for(Int_t j=0;j<8;j++){
     ...
     sprintf(hname,"hGriffing_alphas_ex%d",j);
     sprintf(htitle,"Energy in Griffin Germanium detector #%d gated on low E #alpha particles",j);
     hGriffing_alpha_ex[j]=new TH1F(hname,htitle,8000/4,0,8000);
  ...
  if(nHits>0){
	...
	n_protons++;
	 ...
	 if(address<4)
       ...
       if(cg->IsInside(SiliconE,SilicondE)&&SiliconE>1500){
	 hGriffing_alpha_ex[address]->Fill(Energy);
	 if(address<4)
	   hGriffin0g_alpha_ex->Fill(Energy);
	 else
	   hGriffin90g_alpha_ex->Fill(Energy);
       }
	...
	if(Energy>0){
	 ...
	 if(Detector==1&&Energy>5240&&Energy<5300&&SiliconE>0&&SilicondE>0)
	   PID_gated_5292->Fill(SiliconE,SilicondE);
	 else if(Detector==1&&Energy>5500&&Energy<5560&&SiliconE>0&&SilicondE>0)
	   PID_gated_5540->Fill(SiliconE,SilicondE);
	 else if(Detector==1&&Energy>5340&&Energy<5400&&SiliconE>0&&SilicondE>0)
   ...
   c22->Divide(1,2);
   c22->cd(1);
   PID_gated_5540->Draw("colz");
   c22->cd(2);
   PID_gated_5292->Draw("colz");
